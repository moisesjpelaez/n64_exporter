...
class N64AutoloadMacro {
    ...
    /**
     * Called via --macro from khafile to register N64 macro processing.
     * This adds @:build metadata to the user's arm package for both
     * autoload and trait macros.
     */
    public static function register():Void {
        #if macro
        // Add autoload build macro to all types in the 'arm' package (user code)
        ...
        // Also add trait build macro to all types in the 'arm' package.
        // N64TraitMacro.build() checks if the class extends iron.Trait
        // and skips non-trait classes.
        haxe.macro.Compiler.addGlobalMetadata(
            "arm",  // user package
            "@:build(armory.n64.N64TraitMacro.build())",
            true,   // recursive
            true,   // toTypes
            false   // toFields
        );
